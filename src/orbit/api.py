...
class Handler(BaseHTTPRequestHandler):
    ...
    def _enrollment_html(self, slug, page):
        ...
        <div class="brand">{merchant}</div><h1>{headline}</h1><div class="offer">Get {html.escape(label)} your next purchase</div><form id="join"><label for="phone">Mobile phone number</label><input id="phone" type="tel" inputmode="tel" autocomplete="tel" placeholder="+1 555 123 4567" required><label class="consent"><input id="consent" type="checkbox" required><span>I accept Orbit’s Terms and agree to receive texts from {merchant}, including this offer. Message and data rates may apply. Reply STOP to opt out.</span></label><small>{terms}</small><button id="submit" type="submit" disabled>Text me the promo code</button></form><div id="result" role="status"></div>
        <script>const form=document.getElementById('join'),result=document.getElementById('result'),button=document.getElementById('submit'),consent=document.getElementById('consent');consent.addEventListener('change',()=>{{button.disabled=!consent.checked}});form.addEventListener('submit',async(e)=>{{e.preventDefault();if(!consent.checked)return;button.disabled=true;result.className='';result.textContent='';const claim=new URLSearchParams(location.search).get('claim_token');try{{const response=await fetch({json.dumps(endpoint)},{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{phone:document.getElementById('phone').value,accept_terms:consent.checked,sms_consent:consent.checked,terms_version:{json.dumps(terms_version)},claim_token:claim||undefined}})}});const data=await response.json();if(!response.ok)throw new Error(data.error||'Unable to send offer');result.className='ok';result.textContent=data.duplicate?'Your offer was already requested. Check your messages.':'Your promo code is on its way.';form.hidden=true}}catch(error){{result.className='error';result.textContent=error.message;button.disabled=!consent.checked}}}});</script></main></body></html>"""
    def _raw_body(self):
