...
class Handler(BaseHTTPRequestHandler):
    ...
    def _enrollment_html(self, slug, page):
        ...
        <div class="brand">{merchant}</div><h1>{headline}</h1><div class="offer">Get {html.escape(label)} your next purchase</div><form id="join"><label for="phone">Mobile phone number</label><input id="phone" type="tel" inputmode="tel" autocomplete="tel" placeholder="+1 555 123 4567" required><label class="consent"><input id="terms" type="checkbox" required><span>I accept OrbitGuest’s <a href="/terms" target="_blank" rel="noopener">Terms</a> and acknowledge its <a href="/privacy" target="_blank" rel="noopener">Privacy Policy</a>.</span></label><label class="consent"><input id="sms-consent" type="checkbox"><span>Optional: I agree to receive recurring marketing and promotional text messages from {merchant}, sent using OrbitGuest, including this welcome offer. Message frequency varies. Message and data rates may apply. Reply HELP for help or STOP to opt out. Consent is not a condition of purchase.</span></label><small>Your mobile information will not be sold or shared with third parties for promotional or marketing purposes. {terms}</small><button id="submit" type="submit">Continue</button></form><div id="result" role="status"></div>
        <script>const form=document.getElementById('join'),result=document.getElementById('result'),button=document.getElementById('submit'),terms=document.getElementById('terms'),sms=document.getElementById('sms-consent');form.addEventListener('submit',async(e)=>{{e.preventDefault();button.disabled=true;result.className='';result.textContent='';const claim=new URLSearchParams(location.search).get('claim_token');try{{const response=await fetch({json.dumps(endpoint)},{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{phone:document.getElementById('phone').value,accept_terms:terms.checked,sms_consent:sms.checked,terms_version:{json.dumps(terms_version)},claim_token:claim||undefined}})}});const data=await response.json();if(!response.ok)throw new Error(data.error||'Unable to process request');result.className='ok';result.textContent=data.status==='consent_declined'?'No text messages will be sent.':(data.duplicate?'Your offer was already requested. Check your messages.':'Your promo code is on its way.');form.hidden=true}}catch(error){{result.className='error';result.textContent=error.message;button.disabled=false}}}});</script></main></body></html>"""
    ...
    def _policy_html(self, policy):
        if policy == "privacy":
            ...
            content = """<p>These terms govern use of OrbitGuest enrollment pages. Restaurant offers are subject to the terms displayed on the applicable restaurant page.</p><h2>Text messages</h2><p>Marketing text consent is optional and is not a condition of purchase. Message frequency varies. Message and data rates may apply. Reply HELP for help or STOP to opt out.</p><h2>Contact</h2><p>Questions may be sent to support@orbitguest.com.</p>"""
